...
def get_families(project):
    """Returns all the FPGA families present in the src dir.
    ...
    """
    path = os.path.join(src_dir, project)
    return next(os.walk(path))[1]


def get_devices(project, family):
    """Returns all the FPGA devices present in the src dir.
    ...
    """
    path = os.path.join(src_dir, project, family)
    return next(os.walk(path))[1]


def get_packages(project, family, device):
    """Returns all the device packages present in the src dir
    ...
    """
    path = os.path.join(src_dir, project, family, device)
    return next(os.walk(path))[1]


def get_boards(project, family, device, package):
    """Returns all the boards present in the src dir.
    ...
    Example:
        ...
        - return value: ['arty']
    """
    path = os.path.join(src_dir, project, family, device, package)
    return next(os.walk(path))[1]
...
def iter_options(args):
    """Yields all the possible combination of:
        - projects,
        - toolchains,
        - families,
        - devices,
    ...
    A combination is valid if the caller verifies that a constraint file is present
    in the path generated by combining all the options:

    Example:
    - path structure:    src/<project>/<family>/<device>/<package>/<board>/<toolchain>.<constraint>
    - valid combination: src/oneblink/xc7/a35t/csg324-1/arty/vpr.pcf
    """

    # Iterate through all project or the user-selected one
    for project in user_selected(args.project) or get_projects():

        # Iterate through all toolchains or the user-selected one
        for toolchain in user_selected(args.toolchain) or get_toolchains():

            # Iterate through all families or the user-selected one
            for family in user_selected(args.family) or get_families(project):

                # Iterate through all devices or the user-selected one
                for device in user_selected(args.device) or get_devices(
                        project, family):

                    # Iterate through all packages or the user-selected one
                    for package in user_selected(args.package) or get_packages(
                            project, family, device):

                        # Iterate through all boards or the user-selected one
